...
object Compositions {
  ...
  def main(args: Array[String]): Unit = {
    val num: Int = 7
    println("a) " + Compositions.testCompose((number: Int) => number.toDouble)((number: Double) => number.toLong)((number: Long) => number.toString)(num))
    println("b) " + Compositions.testMapFlatMap((number: Int) => Some(number))((number: Int) => Some(number * number))((number: Int) => number * number * number)(Some(2)))
    println("c) " + Compositions.testForComprehension((number: Int) => Some(number + number))((number: Int) => Some(number + number))((number: Int) => number + number)(Some(2)))
  }
}
