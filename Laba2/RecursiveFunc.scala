...
object RecursiveFunctions {
  ...
  def flatMap[A, B](list: List[A])(f: A => List[B]): List[B] = {
    @tailrec
    def loop(rem: List[A], agg: List[B]): List[B] = rem match {
      case Cons(n, tail) => loop(tail, testAppend(f(n), agg))
      case Nil() => agg
    }
    testReverse(loop(list, new Nil[B]))
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    testReverse(list1)
    println("a)" + RecursiveFunctions.testReverse(list1))
    println("b) " + RecursiveFunctions.testMap(list1, (number: Int) => number + 1))
    println("c) " + RecursiveFunctions.testAppend(list1, list2))
    println("d) " + RecursiveFunctions.testFlatMap(list1,(x: Int) => Cons(x * 3, Nil())))
  }
}
