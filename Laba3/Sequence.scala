...
object Sequence {
  ...
  def testZip[A](a: Seq[A], b: Seq[A]): Seq[(A, A)] = {
    ...
    def loop[A](a: Seq[A], b: Seq[A], c: Seq[(A, A)]): Seq[(A, A)] = a match {
      case a_head +: a_tail => b match {
        case b_last +: Nil => c :+ (a_head, b_last)
        case b_head +: b_tail => loop(a_tail, b_tail, c :+ (a_head, b_head))
      }
      case Nil => c
    }
    loop(a, b, Nil)
  ...
  def testPalindrom[A](seq: Seq[A]): Boolean = {
    ...
    def loop[A] (seq1: Seq[A], seq2: Seq[A]): Boolean = seq1 match{
      ...
      case Nil => seq.equals(seq2.reverse)
    }
    loop(seq, Nil)
  }
